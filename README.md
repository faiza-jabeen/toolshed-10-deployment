...
- [ ] Push to GitHub, deploy both halves, paste the live URLs here
- [ ] **Run Lighthouse on the deployed site before and after** — the fixes above
      are committed, so to get a genuine "before" screenshot, temporarily revert
      the font `media="print"` line and the `manualChunks` block, audit, then
      restore. Screenshot both.
- [ ] Record the video: deployed app on desktop and on a real phone → the
      before/after Lighthouse scores → devtools Network showing the vendor chunk
      served from cache on a second visit → `curl -I` showing the security headers
